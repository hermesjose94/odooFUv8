...
class ir_filters(osv.osv):
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        if not hasattr(ids, '__iter__'):
            ids = [ids]
        res = super(ir_filters, self).read(cr, SUPERUSER_ID, ids, fields=None, context=None, load='_classic_read')
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(ir_filters, self).copy(cr, SUPERUSER_ID, id, default, context)
    ...
    def get_filters(self, cr, uid, model, action_id=None):
        ...
        """
        # available filters: private filters (user_id=SUPERUSER_ID) and public filters (SUPERUSER_ID=NULL),
        ...
        action_domain = self._get_action_domain(cr, SUPERUSER_ID, action_id)
        filter_ids = self.search(cr, SUPERUSER_ID, action_domain +
            [('model_id','=',model),('user_id','in',[SUPERUSER_ID, False])])
        my_filters = self.read(cr, SUPERUSER_ID, filter_ids,
    ...
    def _check_global_default(self, cr, uid, vals, matching_filters, context=None):
        """ _check_global_default(cursor, SUPERUSER_ID, dict, list(dict), dict) -> None
        ...
        """
        action_domain = self._get_action_domain(cr, SUPERUSER_ID, vals.get('action_id'))
        existing_default = self.search(cr, SUPERUSER_ID, action_domain + [
    ...
    def create_or_replace(self, cr, uid, vals, context=None):
        ...
        current_filters = self.get_filters(cr, SUPERUSER_ID, vals['model_id'], action_id)
        ...
        if vals.get('is_default'):
            if vals.get('user_id'):
                ...
                action_domain = self._get_action_domain(cr, SUPERUSER_ID, action_id)
                act_ids = self.search(cr, SUPERUSER_ID, action_domain + [
                        ('model_id', '=', vals['model_id']),
                        ('user_id', '=', vals['user_id']),
                        ('is_default', '=', True),
                    ], context=context)
                if act_ids:
                    self.write(cr, SUPERUSER_ID, act_ids, {'is_default': False}, context=context)
            else:
                self._check_global_default(
                    cr, SUPERUSER_ID, vals, matching_filters, context=None)

        # When a filter exists for the same (name, model, user) triple, we simply
        # replace its definition (considering action_id irrelevant here)
        if matching_filters:
            self.write(cr, SUPERUSER_ID, matching_filters[0]['id'], vals, context)
            return matching_filters[0]['id']

        return self.create(cr, SUPERUSER_ID, vals, context)
    ...
    _defaults = {
        'domain': '[]',
        'context':'{}',
        'user_id': lambda self,cr,SUPERUSER_ID,context=None: SUPERUSER_ID,
        'is_default': False
    }
