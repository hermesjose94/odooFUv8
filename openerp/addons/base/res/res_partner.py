...
class res_partner(osv.Model, format_address):
    ...
    _columns = {
        ...
        'user_id': fields.many2one('res.users', 'Responsable', help='The internal user that is in charge of communicating with this contact if any.'),
        'vat': fields.char('RIF', help="Tax Identification Number. Check the box if this contact is subjected to taxes. Used by the some of the legal statements.", required=False),
        ...
        'code': fields.char('Codigo Cliente', required=True),
        'type_provedor': fields.selection([('nac', 'NACIONAL'), ('int', 'INTERNACIONAL')], 'Tipo de Proveedor'),
        'ci': fields.char('CI'),
    }
    ...
    def onchange_code(self, cr, uid, ids, code, context=None):
        ...
        res={}
        if code:
            code_id = self.search(cr, uid, [('code', '=', code)])
            if code_id:
                was = {
                    'title': _("Advertencia!!"),
                    'message': _(' El Codigo Ya existe      ')
                }
                res = {'value': res, 'warning': was}
            else:
 ...
 #       return self.env['res.users']._user_default_get('res.partner')
    ...
    _defaults = {
        ...
        'image': False,
        'user_id':  _get_user,
        #'user_id': lambda self, cr, uid, c: self.env.user,
        #'user_id': lambda self, cr, uid, c: self.env.user,
        #'user_id': lambda s, cr, uid, c: s.pool.get('res.users')._users_default_get(cr, uid, 'Users', context=c),
        #'user_id': lambda s, cr, uid, c: uid
        #'user_id': lambda self, cr, uid, context:_
