...
class AccountInvoiceRefund(osv.osv_memory):
    ...
    #rsosa, ID 81
    def change_nro_ctrl(self, cr, uid, ids, num_control, context=None):
        res={'value': {'nro_ctrl': False}}
        if not num_control:
            return False
        invoice_id = context.get('active_id')
        invoice_obj = self.pool.get('account.invoice')
        partner_id = invoice_obj.browse(cr, uid, invoice_id, context=None).partner_id 
        invoice_srch = invoice_obj.search(cr, uid, [('type', 'in', ['in_invoice', 'out_invoice']), ('nro_ctrl', '=', num_control), ('partner_id','=',partner_id.id)], context=None)
