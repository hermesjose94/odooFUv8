...
class hr_contract(models.Model):
    ...
    def validate_changed_fields(self, cr, uid, ids, values, come_from, context=None):
        ...
        if permiso_no_remunerado_hrs_float_value or validar_value:
            self.onchange_horas(cr, uid, None, permiso_no_remunerado_hrs_float_value, 'permiso_no_remunerado_hrs_float_value',
                                come_from, context=context)
        return True

    @api.v7
    def get_comisiones_value(self, cr, uid, ids, employee_id, context=None):
        print "amployee_id = " + employee_id
        contract = self.pool.get('hr.contract').browse(cr, uid, employee_id)
    ...
    def hr_verify_sheet(self):
        ...
        new_values.update({i: 0.0 for i in
                           ['bono_nocturno_float_value','hrs_no_lab_float_value','inasistencia_injustificada_float_value',
                            'permiso_no_remunerado_dias_float_value','permiso_no_remunerado_hrs_float_value']})
