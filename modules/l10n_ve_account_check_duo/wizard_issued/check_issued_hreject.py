...
class account_issuedcheck_hreject(models.TransientModel):
    ...
    def action_hreject_issued(self):
        ...
        for check in check_objs:
            if check.state != 'handed':
                raise exceptions.except_osv('Check %s selected error' % (check.number),
            ...
            if not (check.voucher_id.journal_id.default_credit_account_id.id or check.voucher_id.journal_id.default_debit_account_id.id):
                raise exceptions.except_osv('Journal %s selected error' % (check.voucher_id.journal_id.id),
            ...
            if wizard.make_expense:
                if wizard.invoice_expense:
                    if  wizard.expense_amount != 0.00 and wizard.expense_account:
                        ...
                        raise exceptions.except_osv(_('Error'),_('You must assign expense account and amount !'))
            ...
            wf_service.trg_validate('account.issued.check', check.id,
                    'handed_hrejected')
