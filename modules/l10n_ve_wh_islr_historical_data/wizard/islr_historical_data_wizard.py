...
class islr_historical_data_report(osv.osv_memory):
    ...
    def _history_x_fiscalyear(self, cr, uid, date1=None,date2=None,periods=None,partner_id=None):
        report_obj = self.pool.get('islr.wh.historical.data')
        ...
        if p_ids:
            res = report_obj.get_sumary_data(cr, partner_id, p_ids)
        for r in res:
        ...
        if not res:
            raise osv.except_osv(('Advertencia!'),('No hay datos para el periodo seleccionado!'))
        return res

    def print_report(self, cr, uid, ids, context=None):
        res = {}
        active_ids = context and context.get('active_ids', []) or False
        data = self.read(cr, uid, ids, context=context)[0]
        partner_data = {}
        partner = self.pool.get('res.partner').browse(cr,uid,active_ids[0])
        partner_data = {
            'name':partner.name,
        ...
        if data['start_period_id'] and data['end_period_id']:
            res = self._history_x_fiscalyear(cr, uid, None, None,[data['start_period_id'][0], data['end_period_id'][0]], active_ids)
