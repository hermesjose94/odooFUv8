...
class AccountWhIvaLine(models.Model):
    ...
    @api.multi
    def load_taxes(self):
        """ Limpie y cargue nuevamente las líneas
            impositivas del comprobante de retención"""
        ...
        for rec in self:
            if rec.invoice_id:
                if rec.retention_id.type == 'out_invoice':
                    rate = partner._find_accounting_partner(
                        rec.invoice_id.partner_id).wh_iva_rate
                else:
                    rate = (rec.invoice_id.company_id.partner_id).wh_iva_rate
        ...
        if invoice:
            ...
            if bool(ret_ids):
                ...
                raise exceptions.except_orm(
                    'Assigned Invoice !',
                    "La factura ya se ha asignado en una retención"
                    " vat code: '%s' !" % (ret.code))
            result.update({
                'name': invoice.name,
        ...
        for obj in self:
            if obj.type in ('out_invoice', 'out_refund'):
                if  not partner._find_accounting_partner(
                    ...
                    raise exceptions.ValidationError(
                        _('El cliente debe ser un agente de retención de IVA.'))
            else: 
                if  not (obj.company_id.partner_id).wh_iva_agent:
                    raise exceptions.ValidationError(
                        _('La compañia debe ser un agente de retención de IVA.'))
        ...
        for record in self:
            ...
            """ if not inv_str:
                raise exceptions.except_orm(
                    ...
                    _("Las siguientes facturas no son del socio seleccionado:"
                      " \n FACT: %s ") % (inv_str)) """
